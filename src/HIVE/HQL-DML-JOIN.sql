...
--加载数据到表中
load data local inpath '/home/zz/bin/hive_sets/employee.txt' into table employee;
load data local inpath '/home/zz/bin/hive_sets/employee_address.txt' into table employee_address;
load data local inpath '/home/zz/bin/hive_sets/employee_connection.txt' into table employee_connection;
...
-------- c）如果每个表在连接子句中使用相同的列，则hive将多个表上的连接转换为单个MR作业
...
-------- d）join时的最后一个表会通过reducer流式传输，并在其中缓冲之前的其他的表，因此，将大表放置在最后有助于减少reducer阶段缓存数据所需要的内存
SELECT a.val, b.val, c.val FROM a JOIN b ON (a.key = b.key1) JOIN c ON (c.key = b.key1)
